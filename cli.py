...
# Default API server URL with port 5001
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                'algorithm': args.strategy  # <-- changed from args.algorithm
            }
        )
        if response.status_code == 201:
            data = response.json()
...
def main():
    ...
    create_pod_parser.add_argument("--strategy", choices=["first-fit", "best-fit", "worst-fit"], default="first-fit",
                                   help="Pod placement strategy")
