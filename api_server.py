...
class PodScheduler:
    ...
    def schedule_pod(self, cpu_requirement, algorithm="first-fit"):
        ...
        if selected_node_id and self.node_manager.add_pod_to_node(selected_node_id, pod_id, cpu_requirement):
            ...
            logger.info(f"Pod {pod_id} scheduled on node {selected_node_id} with {cpu_requirement} CPU cores")
    ...
    def reschedule_pods_from_node(self, failed_node_id):
        ...
        pods_to_reschedule = [pod_id for pod_id, pod_info in self.pods.items() 
        ...
        for pod_id in pods_to_reschedule:
            cpu_requirement = self.pods[pod_id]['cpu_requirement']
            
            # Try to schedule the pod on a healthy node
            new_pod_id, new_node_id = self.schedule_pod(cpu_requirement)
        ...
        # Remove the pods from the failed node in our records
        for pod_id in pods_to_reschedule:
...
def create_pod():
    ...
    node_id = data.get('node_id')  # Optional node_id parameter
    ...
    if algorithm not in ['first-fit', 'best-fit', 'worst-fit']:
        return jsonify({'error': 'Invalid scheduling algorithm'}), 400
    
    # If node_id is specified, try to create pod on that specific node
    if node_id:
        if node_id not in node_manager.nodes:
            return jsonify({'error': f'Node {node_id} not found'}), 404
        if node_manager.nodes[node_id]['status'] != 'healthy':
            return jsonify({'error': f'Node {node_id} is not healthy'}), 400
        if node_manager.nodes[node_id]['available_cores'] < cpu_requirement:
            return jsonify({'error': f'Node {node_id} does not have enough CPU cores'}), 400
        
        pod_id = str(uuid.uuid4())
        if node_manager.add_pod_to_node(node_id, pod_id, cpu_requirement):
            pod_scheduler.pods[pod_id] = {
                'cpu_requirement': cpu_requirement,
                'node_id': node_id
            }
            return jsonify({
            ...
            return jsonify({'error': f'Failed to create pod on node {node_id}'}), 500
    
    # If no node_id specified, use the scheduler
